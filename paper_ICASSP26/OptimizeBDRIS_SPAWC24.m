function [Cfinal, Ctotal, Theta, Qt] = OptimizeBDRIS_SPAWC24(H,F,G,Qt,Rxx,sigma2n,varargin)
...
Ct = real(log2(det(eye(Nrx) + Sit\(Ht*Ht'))));  % capacity
...
while trueMM == 1  %MM (inner) loop
    ...
    Ct = real(log2(det(eye(Nrx) + Sit\(Ht*Ht'))));
